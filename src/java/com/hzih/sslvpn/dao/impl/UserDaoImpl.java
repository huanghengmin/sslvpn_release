...
public class UserDaoImpl extends MyDaoSupport implements UserDao{
    ...
    @Override
    public User findTerminalId(String terminalId) throws Exception {
        String hql="from User user where user.terminal_id ='"+terminalId+"'";
        List<User> users  = super.getHibernateTemplate().find(hql);
        if(users.size()>0&&users!=null){
            return users.get(0);
        }else {
    ...
    @Override
    public User findSimId(String simId) throws Exception {
        String hql="from User user where user.net_id ='"+simId+"'";
        List<User> users  = super.getHibernateTemplate().find(hql);
        if(users.size()>0&&users!=null){
            return users.get(0);
        }else {
    ...
    @Override
    public PageResult findByPages(String cn, String email, String id_card,int disabled, int start, int limit) throws Exception {
        ...
        if (null!=id_card && id_card.length() > 0) {
            hql += " and id_card like ?";
            paramsList.add("%" + id_card + "%");
        }
        if (disabled!=-1) {
            hql += " and enabled = ?";
