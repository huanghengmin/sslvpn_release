...
Ext.onReady(function(){
    ...
    var proxy_ips = new Ext.data.HttpProxy({
        url:"../../SecurityConfigAction_readIps.action"
    });
    ...
    var proxy = new Ext.data.HttpProxy({
        url:"../../SecurityConfigAction_select.action"
    });
    ...
    var colM = new Ext.grid.ColumnModel([
        rowNumber,
		boxM,
        {header:"IP地址",dataIndex:"ip",align:'center',menuDisabled:true,sortable:true,editor:ip_edit}
    ]);
        ...
        handler: function() {
            if(form_1.form.isValid()){
                form_1.getForm().submit({
                    url:'../../SecurityConfigAction_update8443.action',
                    method:'POST',
                    waitTitle :'系统提示',
                    waitMsg :'正在修改,请稍后...',
        ...
        handler: function() {
            if(form_2.form.isValid()){
                form_2.getForm().submit({
                    url:'../../SecurityConfigAction_update8000.action',
                    method:'POST',
                    waitTitle :'系统提示',
                    waitMsg :'正在修改,请稍后...',
                    success: function(form,action) {
    ...
    var panel = new Ext.Panel({
        ...
        items:[{
            xtype:'fieldset',
            title:'管理服务接口设定IP地址（注：地址必须为VPN服务器网口地址）',
            items:[{
                layout:'column',
                width:500,
        ...
        Ext.MessageBox.show({
            ...
            fn:function(e){
         	    if(e=='ok'){
                    ...
                    myMask.show();
         		    Ext.Ajax.request({
         			    url : '../../SecurityConfigAction_delete.action',
        ...
        Ext.MessageBox.show({
           ...
           fn:function(e){
        	   if(e=='ok'){
        		   ...
        		   Ext.Ajax.request({
        			   url : '../../SecurityConfigAction_insert.action',
        			   params :{ipArray : ipArray },
        			   success : function(r,o){
                           var respText = Ext.util.JSON.decode(r.responseText);
                           var msg = respText.msg;
