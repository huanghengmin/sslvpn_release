...
//        boxM,
        rowNumber,
        {header:"用户名", dataIndex:"username",align:'center', sortable:true, menuDisabled:true,sort:true/*,renderer:show_name*/} ,
...
function view(){
    ...
    if (!recode) {
        ...
        Ext.Msg.confirm("提示", "是否要求客户端截屏？", function (sid) {
            if (sid == "yes") {
                Ext.Ajax.request({
                    url:"../../ClientControl_view_user.action",
                    timeout:20 * 60 * 1000,
                    method:"POST",
                    params:{username:recode.get("cn")},
...
function stop_connect(){
    ...
    if (!recode) {
        ...
        Ext.Msg.confirm("提示", "是否阻断用户？", function (sid) {
            if (sid == "yes") {
                Ext.Ajax.request({
                    url:"../../UserPermissionAction_disable_ByCn.action",
                    timeout:20 * 60 * 1000,
                    method:"POST",
                    params:{username:recode.get("cn")},
                    success:function (form, action) {
